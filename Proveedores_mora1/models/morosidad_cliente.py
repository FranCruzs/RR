...
class MorosidadProveedor(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', string='Moneda', invisible=True)  # Ocultamos currency_id
    amount_total = fields.Char(string='Importe Principal')
    ...
    # Campos relacionados
    invoice_payment_term_id = fields.Many2one('account.payment.term', string='Condición Pago')
    ...
    def init(self):
        ...
        query = """
            ...
            SELECT
                ...
                p.id AS partner_id,
                p.name AS partner_name,  -- Nombre del proveedor
                m.id AS move_id,
                m.name AS move_name,     -- Número de comprobante (ej: "A-0001")
                m.invoice_date,
                m.invoice_date_due,
                m.currency_id,
                ...
                REGEXP_REPLACE(
                    TO_CHAR(
                        CASE WHEN m.move_type = 'in_refund' THEN -m.amount_total ELSE m.amount_total END, 
                        'FM9,999,999,990.00'
                    ), 
                    ',', '.', 'g'
                ) AS amount_total,
            ...
            WHERE
                m.move_type IN ('in_invoice', 'in_refund', 'in_debit')  -- Solo documentos que afectan deuda
                AND m.state = 'posted'                                 -- Solo confirmados
                AND m.payment_state IN ('not_paid', 'partial')          -- Pendientes o pagos parciales
                AND m.invoice_date_due < CURRENT_DATE                  -- Solo vencidos
            ORDER BY
                CASE
                    WHEN m.move_type = 'in_refund' THEN -m.amount_residual_signed
                    ELSE m.amount_residual_signed
                END ASC,  -- Ordena por importe_secundario descendente (mayor a menor)
                p.name,    -- Luego por nombre de proveedor
                m.invoice_date_due  -- Finalmente por fecha de vencimiento
